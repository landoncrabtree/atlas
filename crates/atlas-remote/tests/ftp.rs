//! Integration tests for the OpenDAL FTP backend against the
//! pyftpdlib-based mock server in `tools/mock-servers/ftp_server.py`.
//!
//! Run with `cargo test -p atlas-remote --test ftp -- --nocapture`.
...
async fn connect_with_password() -> Result<()> {
    crate::skip_if_no_python!();
    let server = MockFtpServer::start_auth("atlas-user", "atlas-pass")?;
    std::fs::write(server.root_dir().join("probe.txt"), b"hi")?;

    let vm = open_vm(server.uri(), Credentials::Password("atlas-pass".into()))?;
    // OpenDAL synthesises a DIR metadata for path "/", so we must hit a
    // real object to force the backend to actually log in.
    let bytes = vm
        .read("probe.txt")
        .await
...
async fn rename_moves_file() -> Result<()> {
    ...
    let vm = open_vm(server.uri(), Credentials::Anonymous)?;
    // OpenDAL's FTP backend doesn't currently implement rename. Verify that
    // the error surfaces cleanly as `Unsupported` rather than panicking, and
    // fall back to copy+delete as a smoke test for the underlying primitives.
